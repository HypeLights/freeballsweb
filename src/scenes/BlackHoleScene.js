...
export class BlackHoleScene extends Scene {
    init(data, dataUint) {
        ...
        for (let i = 1; i < this.solver.particleCount; i++) {
            ...
            // Orbital Velocity for 1/r Gravity (Logarithmic Potential)
            // F = G / r
            // Centripetal Force: mv^2/r = F = G/r
            // v^2 = G
            // v = sqrt(G)
            // Velocity is CONSTANT regardless of distance!

            // We use (dist + 10.0) in shader, so:
            // v = sqrt(G * dist / (dist + 10.0))
            const forceMag = G_shader / (dist + 10.0);
            const v = Math.sqrt(dist * forceMag);
